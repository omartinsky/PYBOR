...
class ConventionsTest(unittest.TestCase):
    def convention_test(self):
        conventions = Conventions.FromSpreadsheet(join(dirname(__file__), 'conventions.txt'))
...
class CurveInterpolationTest(unittest.TestCase):
    def test_curve_linear_logdf(self):
        ...
        aae(c.get_fwd_rate_aligned(arr([1, 1.3, 1.9]), CouponFreq.ZERO, DCC.ACT365), [1.868445, 1.8698797])
        aae(c.get_fwd_rate_aligned(arr([1, 1.3, 1.9]), CouponFreq.ZERO, DCC.ACT365), [1.868445, 1.8698797])
        aae(c.get_fwd_rate(arr([1, 1.3]), arr([1.3, 1.9]), CouponFreq.CONTINUOUS, DCC.ACT365),
            [1.8670117, 1.8670117])
        aae(c.get_zero_rate(arr([1]), CouponFreq.ZERO, DCC.ACT365), [(1 / .98 - 1.) * 365.])
        aae(c.get_zero_rate(arr([1]), CouponFreq.CONTINUOUS, DCC.ACT365), [-np.log(.98) * 365.])
    ...
    def test_curve_cubic_logdf(self):
        self.assertRaises(BaseException, lambda: Curve('libor', arr([0, 1]), arr([1, 0.8])))
        c = Curve('libor', 0, arr([0.001, 1, 2]), arr([.99, .98, .975]), InterpolationMode.CUBIC_LOGDF)
        self.assertEqual(c.get_id(), 'libor')
        aae(c.get_df([0, 1, 2]), [1, .98, .975])
